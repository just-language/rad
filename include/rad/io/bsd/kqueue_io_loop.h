...
namespace RAD_LIB_NAMESPACE::io::detail {
    ...
    struct io_loop_impl {
        ...
        set_timer_timeout(std::chrono::steady_clock::duration timeout);

        RAD_EXPORT_DECL void on_child_fork(std::error_code& ec) noexcept;

        kqueue_handle reactor_fd;
        static constexpr uintptr_t interrupter = 1;
        static constexpr uintptr_t timer = 2;
