...
namespace RAD_LIB_NAMESPACE::net::ssl::openssl {
    /*!
     * @brief Get a const reference to the error category used by OpenSSL
     * ssl implementation.
     * @return A const reference to the error category used by OpenSSL ssl
     * implementation.
     */
    const std::error_category& ssl_category() noexcept;

    /*!
     * @brief OpenSSL context options.
     */
    ...
    /*!
     * @brief OpenSSL context implementation.
     *
     * The implementation wraps wraps `SSL_CTX` openssl context and allows
     * access to it using `native_handle` method.
     *
    ...
    public:
        using ssl_impl_type = openssl_impl;
        ...
        /*!
         * @brief Create an openssl ssl implementation derived
         * from this context. The context must be open prior to
         * this call.
         * @param ec Set to indicate errors occured, if any.
         * @return An openssl ssl implementation derived from
        ...
        /*!
         * @brief Set options on the context.
         * @param opts A bitmask of options to set.
         * @return The old options of the context.
         */
        RAD_EXPORT_DECL options set_options(options opts) noexcept;

        /*!
         * @brief Clear options from the context.
         * @param opts A bitmask of options to clear.
         * @return The old options of the context.
         */
        RAD_EXPORT_DECL options clear_options(options opts) noexcept;

        /*!
         * @brief Get the options of the context.
         * @return The options of the context.
         */
        RAD_EXPORT_DECL options get_options() const noexcept;

        /*!
         * @brief Get the native context pointer.
         * The returned pointer can be casted to `SSL_CTX*`.
         * If there is no current context, the returned pointer
         * is null.
         * @return A pointer to the native context that can be
         * casted to `SSL_CTX*`, or null.
         */
         ...
         * a single certificate. The files must be named using
         * the subject name's hash and an extension of ".0".
         *
    ...
    /*!
     * @brief OpenSSL ssl implementation.
     *
     * The implementation wraps wraps `SSL` openssl ssl session and allows
     * access to it using `native_handle` method.
     */
    class RAD_EXPORT_VTABLE openssl_impl : public ssl_impl_base {
    public:
        using context_type = context;
        using bio_type = ring_bio;

        openssl_impl() = default;

        openssl_impl(context_type& ctx) {
            open(ctx);
        }

        openssl_impl(context_type& ctx, uintptr_t sock_fd) {
            open(ctx);
            set_fd(sock_fd);
        }

        openssl_impl(const openssl_impl&) = delete;

        openssl_impl& operator=(const openssl_impl&) = delete;

        openssl_impl(openssl_impl&& other) noexcept
            : ssl_{std::exchange(other.ssl_, nullptr)} {
        }

        openssl_impl& operator=(openssl_impl&& other) noexcept {
            destroy_ssl();
            ssl_ = std::exchange(other.ssl_, nullptr);
            return *this;
        }
         ...
         * implementation. For this OpenSSL implementation the
         * returned pointer can be casted to SSL*. If the
         * implementation is closed, the returned pointer is
         * null.
         * @return A pointer to the native ssl implementation
         * which can be casted to SSL*.
         */
         ...
         * associated to. It must be an openssl::context,
         ...
         * openssl::context @p ctx and replace the existing one,
